...
public final class PhotoNetworkHooksImpl {
    ...
    public static void uploadChunk(UUID id, int index, byte[] data) {
        NeoGuanNiaoNetwork.sendToServer(new PhotoUploadChunkPacket(id, index, data));
    }

    public static void finishUpload(UUID id) {
        NeoGuanNiao.LOGGER.info("Photo upload finish {}", id);
        NeoGuanNiaoNetwork.sendToServer(new FinishPhotoUploadPacket(id));
    }

    public static void requestPhoto(String id, String hash) {
        NeoGuanNiao.LOGGER.info("Photo request send id={}", id);
        NeoGuanNiaoNetwork.sendToServer(new PhotoRequestPacket(id, hash));
    }
}
