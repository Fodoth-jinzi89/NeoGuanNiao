...
final class BirdCameraBirdDataHelper {
   private BirdCameraBirdDataHelper() {
   }

   static ResourceLocation resource(String path) {
      return ResourceLocation.fromNamespaceAndPath("birdcamera", path);
   }

   static BirdModel model(String id, String geometry) {
      return BirdModel.createDefault().withId(resource(id)).withLocation(resource("geo/" + geometry + ".geo.json"));
   }

   static BirdSkin skin(String id, String texture, BirdSkinRarity rarity) {
      return BirdSkin.createDefault().withId(resource(id)).withLocation(resource("textures/entity/" + texture + ".png")).withRarity(rarity);
   }

   static List<BirdSkin> genderedSkins(String folder, BirdCameraBirdDataHelper.SkinVariant... variants) {
      List<BirdSkin> skins = new ArrayList<>(variants.length * 2);

      for (BirdCameraBirdDataHelper.SkinVariant variant : variants) {
         ...
         skins.add(base.withId(resource(variant.id() + "_male")).withLocation(resource("textures/entity/" + texture + "_male.png")).withFemale(false));
         skins.add(base.withId(resource(variant.id() + "_female")).withLocation(resource("textures/entity/" + texture + "_female.png")).withMale(false));
      }

      return List.copyOf(skins);
   }

   static List<BirdSkin> ageSkins(String folder, BirdCameraBirdDataHelper.AgeSkinVariant... variants) {
      List<BirdSkin> skins = new ArrayList<>(variants.length * 2);

      for (BirdCameraBirdDataHelper.AgeSkinVariant variant : variants) {
         BirdSkin base = skin(variant.id(), folder + "/" + variant.id(), variant.rarity()).withNatureSpawn(variant.natureSpawn()).withUnique(variant.unique());
         skins.add(base.withBaby(false));
         skins.add(
            base.withId(resource(variant.id() + "_baby"))
   ...
   static BirdModelSkinDatum modelData(
      String modelId, String geometry, List<BirdSkin> skins, BirdModelScaleProfile scaleProfile, float shadowRadius, float globalScale
   ) {
   ...
   static BirdSoundDatum sound(int interval, SoundEvent ambient, SoundEvent hurt, SoundEvent death, SoundEvent interact, SoundEvent eat) {
   ...
   static BirdAnimationDatum animation(String model, String file, Map<String, RawAnimation> animations) {
      return animations(Map.of(resource(model), resource("animations/" + file + ".animation.json")), animations);
   }

   static BirdAnimationDatum animations(Map<ResourceLocation, ResourceLocation> modelAnimations, Map<String, RawAnimation> animations) {
      return BirdAnimationDatum.withAnimationIdAndMap(modelAnimations, animations);
   }

   static List<BirdSkin> budgerigarSkins() {
      return genderedSkins(
         "budgerigar",
   ...
   static List<BirdSkin> nightHeronSkins() {
      ...
      skins.add(skin("cheng_he_guang", "night_heron/cheng_he_guang", BirdSkinRarity.UNIQUE).withUnique(true).withNatureSpawn(false).withFemale(false));
      return List.copyOf(skins);
   }
   ...
   static Map<String, RawAnimation> budgerigarAnimations() {
      return Map.of(
         ...
         loop("idle"),
         "preen",
         playThenIdle("idle_diff_1"),
         "curious",
         playThenIdle("idle_diff_2"),
         "dance",
         loop("idle_diff_3"),
         "eat",
         playThenIdle("eat"),
         "sleep",
         playThenLoop("sleep", "sleep_loop"),
         "sleep_loop",
         loop("sleep_loop"),
         "walk",
         loop("walk"),
         "fly",
         loop("fly_flapping_wing_loop")
      );
   }
   ...
   static BirdCameraBirdDataHelper.SkinVariant skinVariant(String id, BirdSkinRarity rarity) {
      return new BirdCameraBirdDataHelper.SkinVariant(id, rarity, true, true, false);
   }

   static BirdCameraBirdDataHelper.SkinVariant specialSkinVariant(String id, BirdSkinRarity rarity, boolean natureSpawn, boolean breed, boolean unique) {
      return new BirdCameraBirdDataHelper.SkinVariant(id, rarity, natureSpawn, breed, unique);
   }

   static BirdCameraBirdDataHelper.AgeSkinVariant ageSkinVariant(String id, BirdSkinRarity rarity, boolean natureSpawn, boolean unique) {
      return new BirdCameraBirdDataHelper.AgeSkinVariant(id, rarity, natureSpawn, unique);
   }
   ...
   static BirdData simpleBird(
      ...
      return BirdData.createDefault()
         .withSound(
            new BirdSoundDatum(
               0.5F, 240, SoundEvents.PARROT_AMBIENT, SoundEvents.PARROT_HURT, SoundEvents.PARROT_DEATH, SoundEvents.PARROT_AMBIENT, SoundEvents.PARROT_EAT
            )
         )
         ...
         .withModel(
            BirdModelSkinDatum.createDefault()
               ...
               .withBirdSkin(genderedSkins(assetName, skins.toArray(BirdCameraBirdDataHelper.SkinVariant[]::new)))
               .withModelScaleProfile(scaleProfile)
               .withShadowRadius(shadowRadius)
               .withGlobalScale(globalScale)
         )
               ...
               animations.entrySet()
                  ...
                  .collect(
                     Collectors.toUnmodifiableMap(
                        Entry::getKey,
                        entry -> entry.getKey().startsWith("idle_") ? playThenLoop(entry.getValue(), animations.get("idle")) : loop(entry.getValue())
                     )
                  )
            )
         );
   ...
   static record AgeSkinVariant(String id, BirdSkinRarity rarity, boolean natureSpawn, boolean unique) {
   }

   static record SkinVariant(String id, BirdSkinRarity rarity, boolean natureSpawn, boolean breed, boolean unique) {
   }
}
