...
public final class BirdBathAttraction {
   ...
   // 已加载浴盆索引：Level -> ChunkKey -> 浴盆集合（按 chunk 索引，避免遍历每个方块，与原版 2.9.1 一致）
   private static final Map<Level, Map<Long, Set<BirdBathBlockEntity>>> LOADED_BATHS = new IdentityHashMap<>();
   ...
   static void registerBath(BirdBathBlockEntity birdBath) {
      ...
      long chunkKey = ChunkPos.asLong(birdBath.getBlockPos().getX() >> 4, birdBath.getBlockPos().getZ() >> 4);
      LOADED_BATHS
         .computeIfAbsent(level, ignored -> new HashMap<>())
         .computeIfAbsent(chunkKey, ignored -> Collections.newSetFromMap(new IdentityHashMap<>()))
         .add(birdBath);
   }
   ...
   public static Vec3 topUsePosition(BirdBathBlockEntity bath) {
      ...
      return new Vec3((double)pos.getX() + 0.5, (double)pos.getY() + 1.42, (double)pos.getZ() + 0.5);
   }

   public static Vec3 topStandPosition(BirdBathBlockEntity bath) {
      BlockPos pos = bath.getBlockPos();
      return new Vec3((double)pos.getX() + 0.5, (double)pos.getY() + 1.52, (double)pos.getZ() + 0.5);
   }

   public static Vec3 edgeStandPosition(BirdBathBlockEntity bath, Vec3 birdPosition) {
      Vec3 center = topUsePosition(bath);
      Vec3 horizontal = new Vec3(birdPosition.x - center.x, 0.0, birdPosition.z - center.z);
      ...
      return new Vec3(center.x + horizontal.x * 0.5, center.y + 0.2, center.z + horizontal.z * 0.5);
   }
   ...
   public static Vec3 edgeApproachPosition(BirdBathBlockEntity bath, Vec3 birdPosition) {
      ...
      BlockPos pos = bath.getBlockPos();
      return new Vec3(center.x + horizontal.x * 1.35, (double)pos.getY() + 0.05, center.z + horizontal.z * 1.35);
   }
   ...
   public static int getCompetitionPriority(Entity bird) {
      if (bird == null) {
         ...
         String key = BuiltInRegistries.ENTITY_TYPE.getKey(bird.getType()).toString();
         if (key.contains("night_heron")) {
            return 4;
         } else if (key.contains("pigeon") || key.contains("dove") || key.contains("collared")) {
            return 3;
         } else if (key.contains("budgerigar") || key.contains("parakeet")) {
            return 2;
         } else {
            return key.contains("sparrow") ? 1 : 1;
         }
      }
   }
}
