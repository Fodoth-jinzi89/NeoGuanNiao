...
public class BirdBathUseGoal extends Goal {
      ...
      if (!this.bird.level().isClientSide() && this.canStart.getAsBoolean()) {
         if (this.bird.getRandom().nextInt(this.scanChance) != 0) {
            ...
            Optional<BirdBathBlockEntity> found = BirdBathAttraction.findNearbyUsableBath(
               this.bird.level(), this.bird.blockPosition(), this.searchRadius, this::canUseBath
            );
            if (found.isEmpty()) {
               return false;
            } else {
               BirdBathBlockEntity bath = found.get();
               if (!BirdBathAttraction.tryClaimUse(bath, this.bird, 100)) {
                  return false;
               } else {
   ...
   public boolean canContinueToUse() {
      return !this.consumed
         ...
         && (this.targetBath.isOccupiedBy(this.bird.getUUID()) || BirdBathAttraction.tryClaimUse(this.targetBath, this.bird, 100));
   }
   ...
   public void tick() {
      if (this.targetBath != null) {
         ...
         if (this.isAtTopUsePosition(usePosition)) {
            ...
            if (!this.trySettleOntoTop(standPosition) && !this.tryHopOntoTop(standPosition)) {
               if (--this.repathTicks <= 0 || this.bird.getNavigation().isDone()) {
                  this.moveToBathTop();
               }

               if (this.totalTicks % 35 == 0) {
                  BirdBathAttraction.tryClaimUse(this.targetBath, this.bird, 100);
               }
            }
         }
      }
   ...
   private boolean tryHopOntoTop(Vec3 standPosition) {
      if (this.targetBath != null && this.mountHopCooldown <= 0) {
         ...
         if (!(feetOffset >= 1.18) && !(horizontalDistanceSqr > 4.6225)) {
            ...
            this.bird.getNavigation().stop();
            this.bird.setDeltaMovement(horizontal.x, 0.22, horizontal.z);
   ...
   private boolean trySettleOntoTop(Vec3 standPosition) {
      if (this.targetBath == null) {
         ...
         double horizontalDistanceSqr = feet.subtract(standPosition).multiply(1.0, 0.0, 1.0).lengthSqr();
         double feetOffset = feet.y - (double)this.targetBath.getBlockPos().getY();
         if (!(horizontalDistanceSqr > 0.9025) && !(feetOffset < 0.92) && !(feetOffset > 1.95)) {
            this.bird.getNavigation().stop();
   ...
   private boolean isAtTopUsePosition(Vec3 usePosition) {
      Vec3 feet = this.bird.position();
      double horizontalDistanceSqr = feet.subtract(usePosition).multiply(1.0, 0.0, 1.0).lengthSqr();
      double feetOffset = feet.y - (double)this.targetBath.getBlockPos().getY();
      return horizontalDistanceSqr <= 0.7225 && feetOffset >= 1.18 && feetOffset <= 1.95;
   }
   ...
   private void startFeedingAnimationIfNeeded() {
      if (!this.feedingAnimationStarted && this.targetBath != null) {
         ...
         if (this.bird instanceof BirdBathFeedingAnimatable animatable) {
            animatable.startBirdBathFeedingAnimation(this.targetBath.getContentType(), 46);
         }
      }
   }
