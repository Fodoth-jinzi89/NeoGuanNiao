...
public class DoveEntity extends SimpleNeoBirdEntity<DoveEntity> {
   public DoveEntity(EntityType<DoveEntity> entityType, Level level) {
      super(
         entityType,
         level,
         (BirdData)BirdCameraBirdData.DOVE,
         BirdControllers.<DoveEntity>builder()
            .birdAnimationController(
               new BirdAnimationController<DoveEntity>() {
                  @Override
   ...
   public static boolean canSpawn(
      ...
      if (!validGround) {
         return false;
      } else if (!AbstractBirdEntity.hasLocalSpawnCapacity(entityType, level, spawnType, pos, (BirdData)BirdCameraBirdData.DOVE)) {
         return false;
      } else {
