...
import net.fodoth.skina.neoguanniao.platform.CameraOpticsHooks;
...
public final class CameraPreviewPostEffect {
    ...
    public static void prepare(float partialTick) {
        ...
        try {
            ...
            if (!opticsPassFailed && ClientConfigHooks.enableOpticsShader()) {
                try {
                    opticsRendered = CameraOpticsHooks.process(mainTarget, (RenderTarget)opticsTarget, state);
                }
            ...
            if (filter == CameraFilter.NONE || !ClientConfigHooks.enableFilterPreview()) {
