...
import net.fodoth.skina.neoguanniao.platform.CameraNetworkHooks;
...
public final class CameraClientCapture {
    ...
    public static boolean shouldHideHands() {
        return (viewfinderOpen || cleanCapturePending) && ClientConfigHooks.hideHand();
    }
    ...
    public static boolean handleMouseScroll(double delta) {
        ...
        if (currentState.focusMode() == CameraFocusMode.MANUAL && minecraft.options.keyShift.isDown()) {
            ...
            double next = currentState.focalLength() + (delta > 0.0 ? 1.0 : -1.0) * ClientConfigHooks.wheelFocusStep() * scrollAmount;
    ...
    public static double currentFovOverride() { return viewfinderOpen ? currentFov() : cleanCapturePending ? pendingCaptureFov : -1.0; }
    ...
    private static void beginCleanCapture(InteractionHand hand, double fov) {
        ...
        minecraft.options.hideGui = ClientConfigHooks.hideGui();
    ...
    private static double focalSensitivityScale() {
        ...
        return Mth.lerp((double)smooth, (double)1.0, ClientConfigHooks.mouseSensitivity() * 0.28);
    }
    ...
    private static void persistCurrentState() {
        ...
        CameraNetworkHooks.sendSettings(viewfinderHand, currentState);
    }
