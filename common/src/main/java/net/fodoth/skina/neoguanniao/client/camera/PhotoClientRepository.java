...
public final class PhotoClientRepository {
    ...
    private static final Map<String, CachedImage> IMAGES = new LinkedHashMap<>(193, 0.75f, true);
    ...
    public static void upload(InteractionHand hand, byte[] png) throws IOException {
        ...
        PhotoNetworkHooks.beginUpload(uploadId, hand, png.length, dimensions.width(), dimensions.height(), hash);
        int offset = 0;
        int index = 0;
        while (offset < png.length) {
            int end = Math.min(offset + 24576, png.length);
            PhotoNetworkHooks.uploadChunk(uploadId, index, Arrays.copyOfRange(png, offset, end));
            offset += 24576;
            ++index;
        }
        PhotoNetworkHooks.finishUpload(uploadId);
    }
