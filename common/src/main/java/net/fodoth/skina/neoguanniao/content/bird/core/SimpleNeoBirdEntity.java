...
public abstract class SimpleNeoBirdEntity<T extends SimpleNeoBirdEntity<T>> extends AbstractBirdEntity<T> {
    ...
    protected SimpleNeoBirdEntity(EntityType<T> entityType, Level level, BirdData birdData,
                                  BirdControllers<T> controllers) {
        super(entityType, level, birdData, controllers);
        initControllers();
    }

    public static AttributeSupplier.Builder createAttributes(double health, double walkSpeed,
                                                             double flyingSpeed, double followRange) {
    ...
    public static boolean canSpawn(EntityType<? extends AbstractBirdEntity<?>> entityType,
                                   ...
                                   BlockPos pos, RandomSource random, BirdData birdData) {
