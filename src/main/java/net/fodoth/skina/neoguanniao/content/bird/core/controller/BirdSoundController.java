 ...
 * </ul>
 *
 * <p>具体声音资源由 {@link net.fodoth.skina.neoguanniao.content.bird.core.data.BirdData}
 * 提供，使不同鸟种能够拥有独立的声音配置。</p>
 *
 * @param bird 当前控制的鸟实体
 */
public record BirdSoundController(AbstractBirdEntity<?> bird) {
     ...
     * 获取鸟的环境声音。
     *
     * <p>该声音通常用于实体随机鸣叫等自然行为。
     * 返回 {@code null} 表示该鸟不会播放环境音。</p>
     *
     * @return 环境声音事件
     */
    public @Nullable SoundEvent getAmbientSound() {
        return bird.getBirdData().ambientSound();
    }
    ...
    public @Nullable SoundEvent getHurtSound(DamageSource source) {
        return bird.getBirdData().hurtSound();
    }
    ...
    public @Nullable SoundEvent getDeathSound() {
        return bird.getBirdData().deathSound();
    }

    /**
     * 获取鸟声音播放时的音调倍率。
     *
     * <p>用于调整同一种声音在不同鸟个体之间的音高差异。</p>
     *
     * @return 声音音调倍率
     */
    public float getVoicePitch() {
        return bird.getBirdData().voicePitch();
    }
    ...
    public int getAmbientSoundInterval() {
        return bird.getBirdData().ambientSoundInterval();
    }

    /**
     * 播放鸟与玩家或环境交互时的声音。
     *
     * <p>用于处理例如喂食、抚摸、驯服等交互行为产生的声音效果。
     * 具体声音类型由鸟种实现决定。</p>
     */
    public @Nullable SoundEvent getInteractionSound() {
        return bird.getBirdData().interactionSound();
    }
