...
public record BirdRoutineController(AbstractBirdEntity<?> bird) {
     ...
     * 判断当前是否处于鸟类活动时间。
     *
     * <p>根据鸟种 {@link BirdData}
     * 中定义的活动时间范围进行判断。
     * 活动时间允许跨越午夜，例如夜行鸟的活动时间可以从夜晚持续到凌晨。</p>
     *
     * @return 当前是否为活动时间
     */
    public boolean isActiveTime() {
        long time = bird.level().getDayTime() % DAY_LENGTH;
        return time >= bird.getBirdData().activeStartTime() || time < bird.getBirdData().activeEndTime();
    }
     ...
     *
     * <p>栖息时间为活动时间之外的时间段，
     * 通常用于触发停留、休息、寻找栖息点等行为。</p>
     *
     * @return 当前是否为栖息时间
     */
    public boolean isRoostTime() {
        long time = bird.level().getDayTime() % DAY_LENGTH;
        return time >= bird.getBirdData().activeEndTime() && time < bird.getBirdData().activeStartTime();
    }
