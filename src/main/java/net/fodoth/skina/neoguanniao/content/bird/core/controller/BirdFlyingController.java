...
public class BirdFlyingController {
    ...
    public boolean isBirdFlightActive() {
        ...
        boolean hasFlightDuration = bird.getTickController().flightDuration > 0;
    ...
    public boolean isBirdFlying() {
        ...
        return bird.getTickController().flightDuration > 0
                || bird.isBirdLanding()
                || !bird.onGround()
                || bird.isInWater()
    ...
    public boolean isFlightInProgress() {
        return bird.getTickController().flightDuration > 0 || bird.isBirdLanding();
    }
    ...
    public void startShortFlight(Vec3 target, boolean fleeing) {
        if (bird.getTickController().flightTicks <= 0 && bird.getTickController().flightDuration <= 0 && !isLandingFlight) {
            isEscapeFlightActive = fleeing;
            flightTarget = target == null ? findAirCruiseTarget(fleeing) : target;
            BirdData birdData = bird.getBirdData();
            bird.getTickController().flightDuration = fleeing
            ...
            bird.getTickController().flyingTime = 0;
            bird.getTickController().hoverRetargetTicks = nextHoverRetargetDelay();
            bird.setNoGravity(true);
            bird.setSilent(false);
            bird.getNavigation().stop();
            bird.getBehaviorStateController().setBehaviorStateFor(fleeing ? BirdBehaviorState.FLEEING : BirdBehaviorState.FLYING,
                    fleeing ? birdData.shortFleeTicks() : birdData.shortFlyTicks());
        }
    }
    ...
    private int nextHoverRetargetDelay() {
        return bird.getBirdData().hoverRetargetMinDelay() + bird.getRandom().nextInt(bird.getBirdData().hoverRetargetDelayVariance());
    }
    ...
    public void startFlybyFlight(Vec3 target) {
        ...
        BirdData birdData = bird.getBirdData();
        bird.getTickController().flightDuration = birdData.minFlybyDuration() + bird.getRandom().nextInt(birdData.flybyDurationVariance() + 1);
        bird.getTickController().flyingTime = 0;
        bird.getTickController().hoverRetargetTicks = birdData.minHoverRetargetTicks() + bird.getRandom().nextInt(birdData.hoverRetargetTicksVariance() + 1);
        bird.getTickController().flightTicks = Math.max(bird.getTickController().flightTicks, birdData.minimumFlightTicks());
        bird.getNavigation().stop();
        bird.setNoGravity(true);
        bird.setSilent(false);
        ...
        Vec3 movement = direction.normalize().scale(birdData.flybyHorizontalSpeed()).add(0, birdData.flybyUpwardSpeed(), 0);
    ...
    public void finishFlight() {
        ...
        bird.getTickController().flightDuration = 0;
        bird.getTickController().flyingTime = 0;
        flightTarget = null;
        bird.getTickController().hoverRetargetTicks = 0;
        isEscapeFlightActive = false;
        isLandingFlight = false;
        bird.setNoGravity(false);
        BirdData birdData = bird.getBirdData();
        ...
        bird.getTickController().flightTicks = wasEscaping
    ...
    public void beginLandingFlight() {
        ...
        if (landingTarget == null) {
            ...
            isEscapeFlightActive = false;
            BirdData birdData = bird.getBirdData();
            bird.getTickController().flightDuration = birdData.landingFlightMinDuration() + bird.getRandom().nextInt(birdData.landingFlightDurationVariance());
    ...
    public void extendCruiseAfterUnsafeLanding() {
        ...
        isEscapeFlightActive = false;
        BirdData birdData = bird.getBirdData();
        bird.getTickController().flightDuration = birdData.unsafeLandingCruiseMinDuration() + bird.getRandom().nextInt(birdData.unsafeLandingCruiseDurationVariance());
    ...
    public void retargetAirCruise(boolean fleeing) {
        ...
        bird.getTickController().hoverRetargetTicks = this.nextHoverRetargetDelay();
    }
    ...
    public boolean canStartAmbientAirCruise() {
        BirdBehaviorState state = bird.getBehaviorStateController().getBehaviorState();
        return bird.getTickController().flightTicks <= 0 && bird.onGround() && bird.getRoutineController().isActiveTime()
                && bird.getNavigation().isDone() && !bird.getEatingController().isEating()
                && !bird.getRoutineController().isSleepingOrRoosting() && state != BirdBehaviorState.FORAGING
                && state != BirdBehaviorState.PERCHING && state != BirdBehaviorState.FOLLOWING
                && state != BirdBehaviorState.SENTINEL && !state.isEscape();
    }
    ...
    public boolean shouldFaceGroundMovement() {
        if (bird.onGround() && !bird.isPassenger() && !bird.isInWater() && !bird.isVehicle()) {
            ...
            if (!state.isAirborne() && state != BirdBehaviorState.EATING
                ...
                return bird.getDeltaMovement().lengthSqr() > bird.getBirdData().walkingSpeedThreshold()
